...
func (km *KeyManager) Verify(token string) (sqlite.APIKey, error) {
	...
	if km.adminKey != "" && token == km.adminKey {
		return sqlite.APIKey{
			ID:     "admin",
			Name:   "admin",
			Scopes: []string{"jobs:read", "jobs:write", "queue:read", "workers:read", "workers:write", "storage:manage", "webhooks:manage", "profiles:read", "plugins:read", "metrics:read", "keys:manage", "config:manage", "cron:manage"},
		}, nil
	}
